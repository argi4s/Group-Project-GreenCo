...
# - outputs lon/lat/alt waypoints for MAVLink (lon first, lat second)
...
# CHANGED: Now (lon_deg, lat_deg) to match MAVLink standard
LonLat = Tuple[float, float]   # (lon_deg, lat_deg)
XY = Tuple[float, float]       # (x_m East, y_m North)

@dataclass(frozen=True)
class Waypoint:
    lon: float  # CHANGED: lon first
    lat: float  # lat second
    alt_m: float
...
# CHANGED: Now expects (lon, lat)
def lonlat_to_xy_m(p: LonLat, origin: LonLat) -> XY:
    lon, lat = p
    lon0, lat0 = origin
...
# CHANGED: Now returns (lon, lat)
def xy_to_lonlat(p: XY, origin: LonLat) -> LonLat:
    x, y = p
    lon0, lat0 = origin
...
def plan_lawnmower(
    search_polygon: List[LonLat],  # CHANGED: Now expects (lon, lat)
    altitude_m: float,
    *,
    origin_lonlat: LonLat,  # CHANGED: Now (lon, lat)
    camera_ref: CameraFootprintRef = CameraFootprintRef(),
    side_overlap: float = 0.30,
    forward_overlap: float = 0.70,
    max_alt_m: Optional[float] = 50.0,
    holes: Optional[List[List[LonLat]]] = None,  # CHANGED: Now (lon, lat)
    margin_m: float = 1.0,
    start_lonlat: Optional[LonLat] = None,  # CHANGED: Now (lon, lat)
    use_along_step_points: bool = False,
) -> List[Waypoint]:
    """
    Returns a boustrophedon (lawnmower) path covering the polygon.
    Input and output use (lon, lat) order for MAVLink compatibility.
    """
    ...
    # convert polygons to local XY - now using lonlat_to_xy_m
    outer_xy = [lonlat_to_xy_m(p, origin_lonlat) for p in search_polygon]
    outer_xy = _ensure_closed(outer_xy)[:-1]  # keep open list for PCA etc.

    holes_xy: List[List[XY]] = []
    if holes:
        for h in holes:
            hx = [lonlat_to_xy_m(p, origin_lonlat) for p in h]
    ...
    if start_lonlat is not None:
        start_xy = lonlat_to_xy_m(start_lonlat, origin_lonlat)
    ...
    for k in range(n_rows):
        ...
        # For each interval, create a segment
    ...
    # rotate back, convert to lon/lat
    waypoints: List[Waypoint] = []
    for p_r in waypoints_xy_r:
        p_xy = _rotate(p_r, -rot)  # undo rot
        lon, lat = xy_to_lonlat(p_xy, origin_lonlat)  # CHANGED: returns (lon, lat)
        waypoints.append(Waypoint(lon=lon, lat=lat, alt_m=altitude_m))  # CHANGED: lon first
    return waypoints
...
def waypoints_to_tuples(wps: Iterable[Waypoint]) -> List[Tuple[float, float, float]]:
    return [(w.lon, w.lat, w.alt_m) for w in wps]  # CHANGED: returns (lon, lat, alt)