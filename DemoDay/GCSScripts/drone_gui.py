...
class DroneGUI:
    ...
    def create_control_panel(self):
        ...
        tk.Button(btn_frame3, text="TEST SQUARE", command=lambda: self.send_cmd("test_square")).pack(side="left")            # Test square flight pattern for the demonstration of comms on flying day
        ...
        # -------------------------
        row += 1
        fence_frame = ttk.LabelFrame(frame, text="Geofence", padding=5)
        fence_frame.grid(row=row, column=0, columnspan=2, sticky="ew", pady=5)

        # File selection row
        f_file_row = ttk.Frame(fence_frame)
        f_file_row.pack(fill="x", pady=2)

        self.fence_file_label = ttk.Label(f_file_row, text="No file selected", width=30)
        self.fence_file_label.pack(side="left", padx=2)

        ttk.Button(f_file_row, text="Browse CSV", command=self.browse_fence_file, width=12).pack(side="left", padx=2)

        # Waypoint count + actions
        f_action_row = ttk.Frame(fence_frame)
        f_action_row.pack(fill="x", pady=2)

        self.fence_count_var = tk.StringVar(value="0 points")
        ttk.Label(f_action_row, textvariable=self.fence_count_var).pack(side="left", padx=5)

        ttk.Button(f_action_row, text=" Upload Fence", command=self.upload_fence, width=15).pack(side="left", padx=2)
        ttk.Button(f_action_row, text="Preview", command=self.preview_fence, width=8).pack(side="left", padx=2)

        ttk.Button(frame, text="Preview All Paths", command=self.preview_all_paths, width=20).grid(row=row+2, column=0, columnspan=2, pady=10)
        ttk.Button(frame, text="Fetch Paths From Pi", command=lambda: self.send_cmd("preview_request"), width=20).grid(row=row+3, column=0, columnspan=2, pady=5)
        ...
        ttk.Button(frame, text="Message Monitor", command=self.open_message_monitor).grid(
            row=row+4, column=0, columnspan=2, pady=5
        )
    ...
    def create_video_panel(self):
        ...
        clear_btn = tk.Button(
            ...
            fg="black",
            font=("Arial", 12, "bold"),
            command=lambda: self.send_cmd("init")   # resets FSM cleanly
        )
    ...
    def browse_fence_file(self):
        filename = filedialog.askopenfilename(
            title="Select Geofence CSV",
            filetypes=[("CSV files", "*.csv"), ("Text files", "*.txt"), ("All files", "*.*")]
        )

        if filename:
            self.fence_filepath = filename
            self.fence_file_label.config(text=filename.split('/')[-1])
            self.load_fence_points(filename)

    def load_fence_points(self, filename):
        self.fence_points = []
        ...
        try:
            with open(filename, 'r') as f:
                ...
                # Support Python list format: [(lon,lat),(lon,lat)]
                    ...
                    try:
                        ...
                        for p in pts:
                            if len(p) == 2:
                                ...
                                self.fence_points.append((lon, lat))
                    except:
                        pass
            ...
            count = len(self.fence_points)
            self.fence_count_var.set(f"{count} points")

            if count > 0:
                self.log_message(f" Loaded {count} geofence points")
            else:
                self.log_message("No valid geofence points found")

        except Exception as e:
            self.log_message(f"Error loading geofence file: {e}")
            self.fence_points = []
            self.fence_count_var.set("0 points")

    def upload_fence(self):
        if not hasattr(self, 'fence_points') or not self.fence_points:
            self.log_message("No geofence points loaded")
            return

        coord_str = ",".join([f"{p[0]},{p[1]}" for p in self.fence_points])

        self.send_cmd("fence_upload", coord_str)
        self.log_message(f"Uploaded geofence with {len(self.fence_points)} points")
    ...
    def load_plb_waypoints(self, filename):
        """Load PLB waypoints from a KML <LinearRing> coordinates block."""
        self.plb_waypoints = []
        ...
        try:
            ...
            # Extract <coordinates> ... </coordinates>
            import re
            match = re.search(r"<coordinates>(.*?)</coordinates>", text, re.S)
            if not match:
                self.log_message("No <coordinates> block found in KML")
                return

            coord_text = match.group(1).strip()

            # Split by whitespace → each entry is "lon,lat,alt"
            entries = coord_text.split()
            ...
            self.plb_waypoints = pts
            count = len(self.plb_waypoints)

            self.plb_count_var.set(f"{count} waypoints loaded")
            self.log_message(f"Loaded {count} PLB waypoints from KML")

        except Exception as e:
            self.log_message(f"Error loading PLB KML: {e}")
            self.plb_waypoints = []
    ...
    def preview_fence(self):
        """Show preview of loaded geofence points in a new window."""
        if not hasattr(self, 'fence_points') or not self.fence_points:
            self.log_message("No geofence points to preview")
            return

        preview = tk.Toplevel(self.root)
        preview.title("Geofence Preview")
        preview.geometry("600x400")
        ...
        text_widget.insert(tk.END, f"Geofence Points ({len(self.fence_points)} total)\n")
        text_widget.insert(tk.END, "="*50 + "\n")
        text_widget.insert(tk.END, "Index\tLongitude\t\tLatitude\n")
        text_widget.insert(tk.END, "-"*50 + "\n")

        for i, (lon, lat) in enumerate(self.fence_points):
            text_widget.insert(tk.END, f"{i+1}\t{lon:.7f}\t{lat:.7f}\n")
    ...
    def preview_all_paths(self):


        win = tk.Toplevel(self.root)
        win.title("Mission Preview")
        win.geometry("800x600")

        fig, ax = plt.subplots(figsize=(7, 6))

        # --- Helper to plot polygons ---
        ...
        # --- Helper to plot waypoint lists ---
        def plot_waypoints(points, color, label):
            if not points:
                return
        ...
        # --- Plot search polygon (AOI) ---
        try:
            from lawnmower import plan_lawnmower
            # If you want to regenerate lawnmower path:
            # origin = (self.backend.mav_telemetry["lon"], self.backend.mav_telemetry["lat"])
            # wps = plan_lawnmower(SEARCH_AREA, altitude_m=20, origin_lonlat=origin)
            # lawn_points = [(wp.lon, wp.lat) for wp in wps]
            # But simpler: use stored mission if you have it
        except:
            pass

        # --- Plot PLB ---
        if hasattr(self, "plb_waypoints"):
            plot_waypoints(self.plb_waypoints, "blue", "PLB Points")

        # --- Plot Geofence ---
        if hasattr(self, "fence_points"):
            plot_polygon(self.fence_points, "red", "Geofence")

                # --- Plot RETURN path ---
        if hasattr(self, "preview_return"):
            plot_polygon(self.preview_return, "green", "RETURN")

        # --- Plot AOI path ---
        if hasattr(self, "preview_aoi"):
            plot_polygon(self.preview_aoi, "purple", "AOI")



        # --- Plot RETURN path ---
        try:
            from drone_fsm_script import RETURN_WPS
            plot_polygon(RETURN_WPS, "green", "Return Path")
        except:
            pass

        # --- Plot AOI path ---
        try:
            from drone_fsm_script import AOI_WPS
            plot_polygon(AOI_WPS, "purple", "AOI Path")
        except:
            pass

        # --- Plot lawnmower path if available ---
        if hasattr(self, "lawnmower_points"):
            plot_waypoints(self.lawnmower_points, "orange", "Lawnmower Path")
    ...
    def on_pi_message(self, msg_type, content):
        ...
        if msg_type == "PREVIEW":
            self.handle_preview_data(content)
            return
    ...
    def open_message_monitor(self):
        win = tk.Toplevel(self.root)
        win.title("Message Timing Monitor")
        win.geometry("400x600")

        self.monitor_text = tk.Text(win, font=("Courier", 10))
        self.monitor_text.pack(fill="both", expand=True)

        self.update_message_monitor()

    def update_message_monitor(self):
        if not hasattr(self, "monitor_text"):
            return

        now = time.time()

        self.monitor_text.delete("1.0", tk.END)
        self.monitor_text.insert(tk.END, "=== RECEIVED FROM PI ===\n")

        for msg_type, ts in self.backend.last_received.items():
            age = now - ts
            self.monitor_text.insert(tk.END, f"{msg_type:12s}  {age:6.2f} s ago\n")

        self.monitor_text.insert(tk.END, "\n=== SENT TO PI ===\n")

        for event, ts in self.backend.last_sent.items():
            age = now - ts
            self.monitor_text.insert(tk.END, f"{event:12s}  {age:6.2f} s ago\n")

        self.monitor_text.after(500, self.update_message_monitor)


    def handle_preview_data(self, content):
        try:
            kind, data = content.split(" ", 1)
        except:
            return
        ...
        if kind == "LAWN":
            self.lawnmower_points = pts
            self.log_message(f"Received {len(pts)} lawnmower points")

        elif kind == "AOI":
            self.preview_aoi = pts
            self.log_message(f"Received AOI polygon")

        elif kind == "RETURN":
            self.preview_return = pts
            self.log_message(f"Received RETURN path")

        elif kind == "PLB":
            self.preview_plb = pts
            self.log_message(f"Received PLB points")

        elif kind == "GEOFENCE":
            self.preview_geofence = pts
            self.log_message(f"Received geofence polygon")
