...
# The purpose of this code is to send key words via UDP to the FSM on the raspberry PI.
# The FSM on the raspberry Pi then returns FSM messages in response
...
##################################################################################
"""
The get_local_ip() function was an experiment to attempt to avoid needing to identify the host machine via IP address should the router change settings
or if the experiment is being performed in a simulation context at home or in the labs
While the experiment was successful this code did add an overhead to UDP comms as it used mDNS, as such it was retired for this flight to attempt to find out whether
this was the cause of timing issues

Following the actual flight this was not found to be the cause.
"""
...
##################################################################################
"""
Class definition, this is imported and used by drone_gui.py to exact the functions of each class function when called by the press of each button
"""
...
class DroneBackend:
    ...
    def __init__(self, callbacks=None):
        ...
        self.VALID_EVENTS = [
            ...
            "plb_upload", "plb_start", "return", "upl", "deploy", "alt"
        ]
        ...
        self.last_received = {}   # msg_type -> timestamp
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if text.startswith("SEQ="):
                    ...
                    self.last_received[msg_type] = time.time()
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if msg_type == "HEARTBEAT":
                    if msg.get_srcSystem() == 1 and msg.get_srcComponent() == 1:        # Flickering of MODE corrected by filtering source system and component such that only 1,1 is displayed
                        mode_val = msg.custom_mode                                      # Mavlink messages from other connected Mavproxy objects report status otherwise; this does not appear in simulation
    ...
    def send_command(self, event, param=None):
        ...
        self.last_sent[event] = time.time()
