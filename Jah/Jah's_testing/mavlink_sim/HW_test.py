...
mav = mavutil.mavlink_connection("udp:192.168.1.25:14550") # check comunication protocol and port
mav.wait_heartbeat()
...
while True:
    ...
    if cmd == "exit":
        ...
        if wait_mode(mode):
            print(f"[OK] Mode changed to {mode}")
        else:
            print(f"[FAIL] Mode change to {mode} timed out")
        ...
        if ack == mavutil.mavlink.MAV_RESULT_ACCEPTED:
            print("[ACK] Disarm accepted")
        else:
            print("[ACK] Disarm failed:", ack)
